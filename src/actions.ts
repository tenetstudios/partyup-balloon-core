...
export function applyGameAction(room: BalloonRoom, action: GameAction): GameActionResult {
  ...
  if (action.type === "PLACE_WALL") return validationResult(action.type, placeWall(room, action.wall));
  if (action.type === "PLACE_NAILS") return validationResult(action.type, placeNailStrip(room, action.wallSegmentId));
