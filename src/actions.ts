...
export function applyGameAction(room: BalloonRoom, action: GameAction, targetRoom?: BalloonRoom): GameActionResult {
  ...
  if (action.type === "PLACE_WALL") {
    ...
    if (room.economy.coins < cost) return insufficientCoins(action.type, cost);
  ...
  if (action.type === "PLACE_NAILS") {
    ...
    if (!validation.valid) return validationResult(action.type, validation);
    if (room.economy.coins < NAIL_STRIP_COST) return insufficientCoins(action.type, NAIL_STRIP_COST);
  ...
  if (action.type === "REPAIR_WALL") return validationResult(action.type, repairWall(room, action.wallSegmentId));
...
function insufficientCoins(action: GameAction["type"], cost: number): GameActionResult {
  return { action, applied: false, code: "insufficient_coins", message: `Not enough Coins (need ${cost})` };
}

function validationResult(action: GameAction["type"], result: { valid: boolean; code: string; message: string }, successMessage = result.message): GameActionResult {
  return result.valid
