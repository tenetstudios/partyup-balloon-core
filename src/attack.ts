...
export function enqueueBalloon(senderRoom: BalloonRoom, action: SendBalloonAction): QueuedBalloon {
  const queued: QueuedBalloon = {
    id: action.balloonId,
...
export function applyLaunchQueue(senderRoom: BalloonRoom, targetRoom: BalloonRoom, simulationTimeMs: number): LaunchQueueResult {
  ...
  if (!result.valid || !result.balloon) return result;
